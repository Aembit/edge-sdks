...
"""Public GCP Identity Token Trust Provider surface."""
...
DEFAULT_PROVIDER_ID = "gcp-identity-token"


@dataclass(slots=True)
class GcpIdentityTokenTrustProvider:
    """Built-in GCP Identity Token Trust Provider.

    This provider collects the `client.gcp.identityToken` payload content
    ...
    kind: ClassVar[TrustProviderKind] = "gcp_identity_token"
    ...
    def collect_identity(self) -> CollectedTrustProviderIdentity:
        """Collect the GCP identity token for `/edge/v1/auth`."""
        if callable(self.identity_token):
            try:
                token = self.identity_token()
            except Exception as e:
                raise TrustProviderError(
                    f"GCP Identity Token Trust Provider failed to resolve token from source: {e}",
                    retryable=False,
                ) from e
        else:
            token = self.identity_token
        ...
        return CollectedTrustProviderIdentity(client={"gcp": {"identityToken": token_clean}})
